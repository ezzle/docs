---
frametype: "simple_instance"
name: "KB_709611_Class51"
type: "Technology_Function"
supertype: ["Technology_Logical", "Technology_Layer", "EA_Class", ":THING", "EA_Graph_Management", "EA_System"]
own_slot_value: [{slot_reference: "name", value: [{value_type: "string", value_content: "&lt; IP  Telefonie      Telephonie IP&gt;::&lt;  Fax (versturen en ontvangen van faxberichten via mail)          Fax (envoie et réception de fax via mail)&gt;"}, ]}, 
{slot_reference: "system_is_published", value: [{value_type: "boolean", value_content: "true"}, ]}]
---
